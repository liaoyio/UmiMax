...
const icons = (
  theme: 'suggestion' | 'direction' | 'editor' | 'data' | 'logo' | 'other',
) => {
        ...
        {icons('suggestion').map((item, key) => (
          <Col key={key}>
            <Radio.Button value={item}>
              {React.createElement(allIcons[item])}
            </Radio.Button>
          </Col>
        ))}
      </Row>
...
export default (props: { config: any; form: any; schema: any }) => {
  ...
  return (
    <>
      <Input
        addonAfter={
          allIcons[config.value] ? (
            React.createElement(allIcons[config.value], {
              onClick: () => setIconShow(true),
            })
          ) : (
            <span onClick={() => setIconShow(true)}>请选择</span>
          )
        }
        value={config.value}
      />
